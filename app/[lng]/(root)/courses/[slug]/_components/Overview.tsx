...
import { BadgeCheck, CalendarRange, Dot, ListOrdered, MonitorPlay, Star } from 'lucide-react'
	...
	return (
		<>
			...
			<div className={'mt-8 rounded-md bg-secondary p-4 lg:p-6'}>
				...
				<div className={'mt-2'}>
					{formWhom.split(', ').map(i => (
						<div key={i} className={'flex mt-1 items-center'}>
							<Dot />
							<p className={'flex-1 text-gray-400'}>{i}</p>
						</div>
					))}
				</div>
			</div>
...
const learn = 'Ajax, JS, Algoritm, Promise, GitHub, Json-server'
const formWhom = 'dasturlashga qiziqish bulgan , JS dasturlash tilini o\'rganish istagi, Amaliy loyihalar qilish'
